...
function showPatients() {
    ...
    tableIns = table.render({
        ...
        cols: [[
            ...
            {field: 'treatment_fee', title: '手术费用', width: '10%', unresize: true},
            {
                ...
                width: '10%',
                unresize: true,
                toolbar: '<div><button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="allot">分配病房</button></div>'
            }
        ]]
    });
    table.on('tool(patient_table)',function (obj) {
        var data = obj.data;
        if(obj.event=='allot'){
            form.val("sickForm",{"patient_cert_code":data.patient_cert_code})
